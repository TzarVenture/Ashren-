...
export default function Categories() {
  ...
  const y1 = useTransform(smoothProgress, [0, 1], [25, -20]);
  const y2 = useTransform(smoothProgress, [0, 1], [40, -35]);
  const y3 = useTransform(smoothProgress, [0, 1], [25, -20]);
          ...
          >
            <Link href={cat.href} className="flex flex-col items-center w-full">
              <motion.div
                ...
                className={`relative w-24 h-24 sm:w-36 sm:h-36 md:w-44 md:h-44 rounded-full p-[2.5px] bg-gradient-to-tr ${
                  cat.borderColor
                } shadow-[0_12px_35px_rgba(0,0,0,0.85)] group-hover:shadow-[0_0_35px_rgba(234,168,56,0.6)] transition-all duration-500 ${
                  idx === 1 ? "scale-105 sm:scale-110" : ""
                }`}
              >

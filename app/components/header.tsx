...
const NAV_ITEMS: NavItem[] = [
  { name: "Home", icon: HomeIcon },
  {
    ...
    icon: Shirt,
    badge: "Trending",
    badgeColor: "bg-amber-500/20 text-[#F4C463] border-amber-500/40",
  },
  ...
  { name: "Support", href: "#footer", icon: HelpCircle },
];
...
export default function Header({ cartCount, onOpenCart, onOpenVideo, isLoaded = true }: HeaderProps) {
  ...
  useEffect(() => {
    ...
    if (pathname === "/") {
      ...
      const handleScroll = () => {
        ...
        const prodEl = document.getElementById("flagship-products");
        const scrollPos = window.scrollY + 200;

        if (prodEl && scrollPos >= prodEl.offsetTop) {
          setActiveNav("Deals");
  ...
  const handleNavClick = (name: string) => {
    ...
    if (name === "Dresses") {
      ...
      if (pathname === "/") {
        document.getElementById("flagship-products")?.scrollIntoView({ behavior: "smooth" });
      } else {
      ...
      if (pathname === "/") {
        ...
        footer ? footer.scrollIntoView({ behavior: "smooth" }) : window.scrollTo({ top: document.body.scrollHeight, behavior: "smooth" });
      } else {
        router.push("/#footer");
      }
    } else if (name === "Home") {
  ...
  return (
    <>
      ...
      >
            ...
            {NAV_ITEMS.map((item, idx) => {
              ...
              return (
                ...
                >
                  <span>{item.name}</span>
                  {item.badge && (
                    <span className={`text-[8.5px] font-bold px-1.5 py-0.2 rounded-full border ${item.badgeColor}`}>
                      {item.badge}
                    </span>
                  )}
                  {isActive && (
        ...
        {/* Mobile Sub-Header Weather Pill */}
            ...
            >
              <div>
                ...
                <div className="mt-5 space-y-1">
                  <p className="text-[10px] uppercase font-bold tracking-widest text-gray-400 px-3 pb-1">Navigation</p>
                  {NAV_ITEMS.map((item) => {
                    const isActive = activeNav === item.name;
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`p-1.5 rounded-xl ${
                              isActive ? "bg-[#EAA838]/20 text-[#F4C463]" : "bg-white/5 text-gray-400"
                            }`}
                          >
                        ...
                        <div className="flex items-center gap-2">
                          {item.badge && (
                            <span className={`text-[8.5px] font-extrabold px-2 py-0.5 rounded-full border ${item.badgeColor}`}>
                              {item.badge}
                            </span>
                          )}
                          <ChevronRight
                            className={`w-3.5 h-3.5 transition-transform ${
                              isActive ? "text-[#EAA838] translate-x-0.5" : "text-gray-500"
                            }`}
                          />
                        </div>
                      </motion.button>
